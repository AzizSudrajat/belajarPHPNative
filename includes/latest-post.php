...
// tampilkan data post
$query = mysqli_query($conn, "select post.*, category.category_name, category.icon from post, category where category.id = post.category_id order by id desc limit $per_page offset $offset");
?>
...
<?php if(mysqli_num_rows($query)) { ?>
  <?php while($row=mysqli_fetch_array($query)) { ?>
    <div class="row latest-post">
      <div class="col-md-3">
        <?php if($row["image"]==""){
          echo "<img src='images/no-image.png' width='88' />";
        }else{ ?>
